package map.interpreter_gui.model.structures.barrier_table;
...
public class BarrierTable implements IBarrierTable {
    ...
    public synchronized Integer add(Integer value) {
        this.internalDictionary.put(this.nextFree, new Pair<>(value, new ArrayList<>()));
    ...
    public synchronized Map<Integer, Pair<Integer, String>> getContentString() {
        ...
        for (Map.Entry<Integer, Pair<Integer, List<Integer>>> pair : this.internalDictionary.entrySet())
            representation.put(pair.getKey(), new Pair<>(pair.getValue().getKey(), pair.getValue().getValue().toString()));
